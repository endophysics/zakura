...
- Store verified private transactions in a separate in-memory pool.
...
Reject private admission when the transaction depends on an unconfirmed private parent. The initial implementation does not support dependency-closed private retention or promotion.
