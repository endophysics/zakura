...
const FIRST_ADMISSION_ID: AdmissionId = AdmissionId(0);
...
fn context() -> AdmissionContext {
    AdmissionContext {
        admission_id: FIRST_ADMISSION_ID,
...
async fn accepted_private_submission_waits_for_completion() {
    ...
    let call = tokio::spawn(async move { rpc.send_private_transaction(raw).await });
    mempool
...
async fn invalid_private_transaction_never_calls_mempool() {
    ...
    let malformed = rpc
        .send_private_transaction("private-plaintext".to_owned())
        .await;
    let invalid_transaction = rpc.send_private_transaction("00".to_owned()).await;
